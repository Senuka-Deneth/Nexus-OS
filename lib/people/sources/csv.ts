...
export const csvSource: CandidateSource = {
  id: "csv",
  label: "CSV import",
  defaultConsent: "owner_imported",
  normalize(raw: unknown) {
    if (!isRecord(raw)) return sourceFail("CSV source expects a planned row object");
    ...
    const metadata: Record<string, unknown> = { adapter: "csv" };
    if (typedSource && typedSource !== "csv") {
      metadata.original_source = typedSource;
    }
    if (email.data) {
      metadata.external_id = email.data.toLowerCase();
    }
    ...
    const record: NormalizedCandidate = {
      ...
      notes: notes.data,
      external_id:
        typeof parsedMeta.data.external_id === "string"
          ? parsedMeta.data.external_id
          : null,
    };
